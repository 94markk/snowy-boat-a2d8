...
final class DIP_Google_Provider implements DIP_Provider {
    ...
    public function is_configured(array $settings) {
        return ($settings['enabled'] ?? 'no') === 'yes' && DIP_Request::is_secure() && !empty($settings['client_id']) && !empty($settings['client_secret']);
    }
...
final class DIP_Microsoft_Provider implements DIP_Provider {
    ...
    public function callback_url() { return $this->callback; }
    public function is_configured(array $settings) {
        return ($settings['microsoft_enabled'] ?? 'no') === 'yes' && DIP_Request::is_secure() && !empty($settings['microsoft_client_id']) && !empty($settings['microsoft_client_secret']);
    }
