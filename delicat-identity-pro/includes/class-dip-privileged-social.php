...
final class DIP_Privileged_Social {
    ...
    public static function can_link_current($user_id, $provider) {
        ...
        if (!DIP_Request::is_secure()) return false;
    ...
    public static function validate_admin_google_login($user_id, $provider, array $profile) {
        ...
        if (!DIP_Request::is_secure()) return new WP_Error('admin_google_https_required', __('HTTPS est requis pour la connexion Google Administrateur.', 'delicat-google-login'));
    ...
    public static function render_security_card($user_id = 0) {
        ...
        <div class="dip-security-card dip-admin-google-secure">
          ...
          <?php if (!$linked && ($settings['enabled'] ?? 'yes') === 'yes' && $totp && $recent && DIP_Request::is_secure()): ?>
          ...
          <?php elseif ($linked && !$approved && $totp && $recent && DIP_Request::is_secure()): ?>
