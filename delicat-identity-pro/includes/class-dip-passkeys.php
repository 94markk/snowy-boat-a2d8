...
final class DIP_Passkeys {
    ...
    private static function https_ok() {
        if (DIP_Request::is_secure()) return true;
    ...
    private static function safe_redirect($value, $fallback = '') {
        ...
        $url = wp_validate_redirect(esc_url_raw($value), $fallback);
        $path = strtolower((string) wp_parse_url($url, PHP_URL_PATH));
        $query = strtolower((string) wp_parse_url($url, PHP_URL_QUERY));
        if (strpos($path, '/wp-login.php') !== false || strpos($path, '/wp-admin') === 0 || strpos($query, 'action=logout') !== false || strpos($query, 'action=lostpassword') !== false || strpos($query, 'action=rp') !== false || strpos($query, 'action=resetpass') !== false) return $fallback;
        return $url;
    }
    ...
    public static function rest_login_verify($request) {
        ...
        wp_set_auth_cookie($uid, !empty($state['remember']), DIP_Request::is_secure());
