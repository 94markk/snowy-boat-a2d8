...
final class DIP_Plugin {
    ...
    public function settings_page() {
        ...
        <div class="wrap dip-admin-wrap">
          ...
          <section id="dip-quick-start" class="dip-quick-grid">
            ...
            <article class="dip-quick-card"><span class="dip-status-dot <?php echo is_ssl() ? 'is-good' : 'is-bad'; ?>"></span><div><small>HTTPS</small><strong><?php echo is_ssl() ? 'Actif' : 'Obligatoire'; ?></strong><p>OAuth et l’API mobile exigent une connexion chiffrée.</p></div></article>
          </section>
    ...
    private function callback() {
        ...
        wp_set_auth_cookie($user_id, $s['remember_login'] === 'yes', is_ssl());
    ...
    private function post_login_redirect($url, $fallback = '') {
        ...
        return DIP_Session_Router::destination($filtered, $fallback);
    }
    ...
    /**
     * The request looks HTTPS to the client but not to PHP. Used only to make
     * the diagnostic accurate; the OAuth gate itself still requires is_ssl().
     */
    public static function proxy_https_detected() {
        if (is_ssl()) return false;
        $proto = strtolower(trim((string) ($_SERVER['HTTP_X_FORWARDED_PROTO'] ?? '')));
        if ($proto !== '' && strpos($proto, 'https') !== false) return true;
        $visitor = (string) ($_SERVER['HTTP_CF_VISITOR'] ?? '');
        return $visitor !== '' && stripos($visitor, 'https') !== false;
    }
