...
final class DIP_Native_Auth {
    ...
    public static function init() {
        ...
        add_filter('style_loader_tag', [__CLASS__, 'defer_modal_style_tag'], 20, 2);
    ...
    private static function safe_redirect($value, $fallback = '') {
        ...
        $value = trim((string) $value);
        if ($value !== '' && strpos($value, '/') === 0 && strpos($value, '//') !== 0) $value = home_url($value);
        return wp_validate_redirect(esc_url_raw($value), $fallback);
    }
    ...
    public static function ajax_register() {
        ...
        wp_set_auth_cookie($user_id, true, DIP_Request::is_secure());
    ...
    /**
     * Does this page need the sign-in dialog painted before anything else?
     *
     * Only when it will open by itself: an e-mail verification landing, a
     * social-login error, a #delicat-login link. On every other page the dialog
     * is hidden until a finger touches a button, which is hundreds of
     * milliseconds after the document has already been painted.
     */
    public static function modal_opens_on_load() {
        foreach (['dip_auth_error', 'dip_verified', 'dip_identity_open', 'dip_identity_eager'] as $flag) {
            if (isset($_GET[$flag])) return true; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- presentation flag.
        }
        if (did_action('login_enqueue_scripts') || (isset($GLOBALS['pagenow']) && $GLOBALS['pagenow'] === 'wp-login.php')) return true;
        return (bool) apply_filters('dip_native_auth_modal_opens_on_load', false);
    }

    /**
     * Take the dialog's stylesheets off the critical path.
     *
     * login.css and identity-modal-v3.css are about 32KB of render-blocking CSS
     * in <head> on every page a signed-out customer opens, for a dialog that is
     * hidden until they ask for it. On a 2G or 3G connection that is a visible
     * delay before ANY of the shop paints. They are fetched at the same
     * priority, applied when they arrive, and are always in place long before a
     * tap can open anything. A page that opens the dialog by itself keeps the
     * blocking load, so it can never appear unstyled.
     */
    public static function defer_modal_style_tag($tag, $handle) {
        if (!in_array($handle, ['dip-identity-modal-v3', 'dip-login', 'dip-passkeys'], true)) return $tag;
        if (is_admin() || self::modal_opens_on_load()) return $tag;
        if (strpos($tag, "media='print'") !== false || strpos($tag, 'media="print"') !== false) return $tag;
        if (!preg_match("/media=['\"][^'\"]*['\"]/", $tag)) return $tag;

        $async = preg_replace("/media=['\"][^'\"]*['\"]/", "media=\"print\" onload=\"this.media='all';this.onload=null\"", $tag, 1);
        /* Anything without JavaScript still gets the stylesheet, blocking. */
        return $async . '<noscript>' . $tag . '</noscript>' . "\n";
    }

    public static function protect_modal_script_tag($tag, $handle) {
        if (!in_array($handle, ['dip-identity-modal-v5','dip-identity-loader'], true)) return $tag;
    ...
    /**
     * The French sentence behind each social-login failure code.
     *
     * This table used to live inside the modal script - roughly four kilobytes
     * of text shipped to every visitor on every page, to be read on the rare
     * page that actually carries an error code. It is needed only when one is
     * present, and it belongs in PHP anyway, where it can be translated.
     */
    public static function auth_error_messages() {
        return [
            'access_denied' => __('Connexion Google annulée ou refusée.', 'delicat-google-login'),
            ...
            'privileged_auto_link_blocked' => __('Ce compte sensible existe déjà. Delicat refuse une simple liaison par e-mail et doit reprendre son lien Google historique ou une liaison sécurisée.', 'delicat-google-login'),
            ...
            'email_exists' => __('Un compte utilise déjà cette adresse. Connectez-vous avec votre mot de passe puis reliez Google depuis votre compte.', 'delicat-google-login'),
            'not_configured' => __('Google Login n’est pas disponible actuellement.', 'delicat-google-login'),
            ...
            'dip_2fa_rate_limited' => __('Trop de tentatives 2FA. Réessayez dans quelques minutes.', 'delicat-google-login'),
            ...
            'admin_google_not_authorized' => __('Cette identité Google doit être approuvée depuis votre Centre de sécurité Administrateur.', 'delicat-google-login'),
            'admin_google_two_factor_required' => __('Activez TOTP 2FA avant d’utiliser Google avec votre compte Administrateur.', 'delicat-google-login'),
    ...
    /** Ship the table only to the page that has an error code to explain. */
    public static function localize_auth_messages($handle) {
        if (empty($_GET['dip_auth_error'])) return; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- presentation only.
        wp_localize_script($handle, 'DIPIdentityMessages', self::auth_error_messages());
    }
    ...
    public static function enqueue_assets() {
        ...
        if (self::should_lazy_modal()) {
            ...
            wp_localize_script('dip-identity-loader', 'DIPIdentityLoader', [
                ...
                'assets' => [
                    'loginCss' => DIP_URL . 'assets/login.css',
                    'modalCss' => DIP_URL . 'assets/identity-modal-v3.css',
                    'modalJs' => DIP_URL . 'assets/identity-modal-v5.js',
                    'passkeysCss' => DIP_URL . 'assets/passkeys.css',
                    'passkeysJs' => DIP_URL . 'assets/passkeys.js',
                ],
        ...
        wp_enqueue_style('dip-identity-modal-v3', DIP_URL . 'assets/identity-modal-v3.css', ['dip-login'], DIP_VERSION);
        wp_enqueue_script('dip-identity-modal-v5', DIP_URL . 'assets/identity-modal-v5.js', [], DIP_VERSION, true);
        self::localize_auth_messages('dip-identity-modal-v5');
        wp_localize_script('dip-identity-modal-v5', 'DIPIdentityModal', [
            'ajaxUrl' => self::ajax_url(),
            'redirectUrl' => self::login_redirect(),
            'nonceAction' => self::AJAX_NONCE_ACTION,
            'loginAction' => self::AJAX_LOGIN_ACTION,
    ...
    private static function modal_markup($redirect_url = '') {
        ...
        <div id="dip-identity-modal" class="dipx-root" data-dip-identity-modal hidden aria-hidden="true">
          ...
          <section class="dipx-card" role="dialog" aria-modal="true" aria-labelledby="dipx-title" aria-describedby="dipx-subtitle" tabindex="-1">
            ...
            <form id="dipx-panel-login" class="dipx-form dipx-form-login" autocomplete="on" novalidate role="tabpanel" aria-labelledby="dipx-tab-login">
              ...
              <div class="dipx-options">
                ...
                <a href="<?php echo esc_url(wp_lostpassword_url()); ?>"><?php echo esc_html__('Mot de passe oublié ?', 'delicat-google-login'); ?></a>
              </div>
            ...
            <?php if ($registration_enabled) : ?>
            <form id="dipx-panel-register" class="dipx-form dipx-form-register" autocomplete="on" novalidate hidden role="tabpanel" aria-labelledby="dipx-tab-register">
