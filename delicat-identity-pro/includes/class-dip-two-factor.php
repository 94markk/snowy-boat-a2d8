...
final class DIP_Two_Factor {
    ...
    public static function enforce_password_second_factor($user, $username, $password) {
        ...
        if (!DIP_Request::is_secure()) return new WP_Error('dip_2fa_https_required', __('HTTPS est requis pour la vérification à deux facteurs.', 'delicat-google-login'));
    ...
    public static function begin_enrollment() {
        ...
        if (!DIP_Request::is_secure()) { wp_safe_redirect(self::security_url('https_required')); exit; }
    ...
    public static function render_settings($user_id = 0) {
        ...
        <div class="dip-security-card dip-2fa-card">
          ...
          <?php elseif (!$enabled && self::enrollment_available() && DIP_Request::is_secure()): ?>
          ...
          <?php elseif (!$enabled): ?>
            <div class="dip-security-notice"><?php echo DIP_Request::is_secure() ? esc_html__('L’administrateur a désactivé les nouveaux enrôlements 2FA.', 'delicat-google-login') : esc_html__('HTTPS est requis avant d’activer 2FA.', 'delicat-google-login'); ?></div>
          <?php else: ?>
    ...
    private static function set_preauth_cookie($token, $expires) {
        ...
        setcookie(self::PREAUTH_COOKIE, $token, [
            ...
            'secure'=>DIP_Request::is_secure(),'httponly'=>true,'samesite'=>'Lax'
        ]);
    ...
    private static function clear_preauth_cookie() {
        if (!headers_sent()) setcookie(self::PREAUTH_COOKIE, '', ['expires'=>time()-3600,'path'=>COOKIEPATH ?: '/','domain'=>COOKIE_DOMAIN ?: '','secure'=>DIP_Request::is_secure(),'httponly'=>true,'samesite'=>'Lax']);
    ...
    public static function begin_web_challenge($user_id, $redirect, $remember = true, $method = 'second_factor') {
        $user_id = absint($user_id);
        if (!$user_id || !self::is_enabled($user_id) || !DIP_Request::is_secure()) return '';
    ...
    public static function maybe_render_challenge() {
        ...
        if (!DIP_Request::is_secure()) { wp_safe_redirect(add_query_arg('dip_auth_error','two_factor_https_required',home_url('/')) . '#delicat-login'); exit; }
        nocache_headers();
    ...
    public static function complete_web_challenge() {
        ...
        wp_set_auth_cookie($user_id, !empty($state['remember']), DIP_Request::is_secure());
