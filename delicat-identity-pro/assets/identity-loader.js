(function(){
  ...
  function css(url,id){
    ...
    return new Promise(function(resolve,reject){
      var done=false,timer,l=document.createElement('link');
      function finish(error){if(done)return;done=true;window.clearTimeout(timer);if(error){l.remove();reject(error)}else resolve()}
      l.id=id;l.rel='stylesheet';l.href=url;l.setAttribute('data-no-optimize','1');
      l.onload=function(){finish()};l.onerror=function(){finish(new Error('style_failed'))};
      timer=window.setTimeout(function(){finish(new Error('style_timeout'))},8000);document.head.appendChild(l);
    });
  }
  ...
  function load(){
    ...
    state.promise=freshModalNonce().then(fragment).then(function(html){
      ...
      var node=document.getElementById(cfg.modalId||'dip-identity-modal');
      var styled=node&&window.getComputedStyle(node).getPropertyValue('--dipx-style-ready').trim()==='1';
      var styles=styled?[]:[css(a.modalCss,'dip-identity-modal-v3-lazy')];
      return Promise.all(styles).then(function(){return script(a.modalJs,'dip-identity-modal-v4-lazy',true)}).then(function(){
