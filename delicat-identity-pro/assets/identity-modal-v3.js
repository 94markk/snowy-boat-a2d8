(function () {
  'use strict';
  if (window.__DIP_IDENTITY_MODAL_V3__) return;
  window.__DIP_IDENTITY_MODAL_V3__ = true;
  ...
  function boot() {
    ...
    function watchLegacy() {
      suppressLegacy();
      if (!Number(cfg.legacyDetected || 0) || typeof MutationObserver === 'undefined') return;
      if (legacyObserver) return;
      legacyObserver = new MutationObserver(suppressLegacy);
      try { legacyObserver.observe(document.documentElement, { childList: true, subtree: true }); } catch (e) {}
    }
    ...
    function refreshNonces(showError) {
      ...
      return request(cfg.ajaxUrl || '/wp-admin/admin-ajax.php', {
        method: 'POST',
        body: data,
    ...
    function openModal(preferredTab) {
      ...
      try {
        if (document.body && root.parentNode === document.body && document.body.lastElementChild !== root) document.body.appendChild(root);
      } catch (e) {}
    ...
    document.addEventListener('click', function (event) {
      var target = event.target && event.target.closest ? event.target.closest('[data-dip-auth-open],[data-dl-open],a[href="#delicat-login"],a[href="#dip-identity-modal"],[aria-controls="dip-identity-modal"]') : null;
      if (target) {
        event.preventDefault();
        event.stopPropagation();
        if (event.stopImmediatePropagation) event.stopImmediatePropagation();
        openModal(target.hasAttribute('data-dip-auth-register') || target.hasAttribute('data-dl-open-register') ? 'register' : 'login');
        return;
      }
