(function(){
  ...
  function request(path,method,body,customer){
    ...
    return fetch(String(cfg.rest).replace(/\/$/,'')+path,{method:method||'POST',credentials:'same-origin',cache:'no-store',headers:headers,body:body===undefined?undefined:JSON.stringify(body)}).then(function(r){
  ...
  function login(button){
    ...
    }).then(function(data){setStatus('Connexion Passkey réussie.','success',root);window.setTimeout(function(){window.location.assign((data&&data.redirect)||cfg.redirect||'/my-account/');},250);}).catch(function(err){setStatus(errorMessage(err),'error',root);}).finally(function(){setBusy(button,false);});
  }
