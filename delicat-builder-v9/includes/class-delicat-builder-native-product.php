...
final class Delicat_Builder_V9_Native_Product {
	...
	private static function render_important( array $m ): void {
		...
		echo '<div class="dnp-important-body"><div class="dnp-important-head">';
	...
	public static function assets(): void {
		...
		wp_enqueue_style('delicat-v9-native-product-pro15',DELICAT_BUILDER_V9_URL.'assets/css/native-product-pro15.css',array('delicat-v9-native-product'),DELICAT_BUILDER_V9_VERSION);
	...
	public static function meta_box(WP_Post $post): void {
		$m=self::product_settings($post->ID); wp_nonce_field('delicat_native_product_save','delicat_native_product_nonce'); ?>
		<p><label>Rendu natif<select name="dnp_product[enabled]" style="width:100%"><option value="inherit" <?php selected($m['enabled'],'inherit'); ?>>Hériter du global</option><option value="1" <?php selected($m['enabled'],'1'); ?>>Activé</option><option value="0" <?php selected($m['enabled'],'0'); ?>>Désactivé</option></select></label></p>
	...
	public static function save_product(int $id,WP_Post $post,bool $update): void {
		...
		$m['hero_image']=esc_url_raw($in['hero_image']??'');
