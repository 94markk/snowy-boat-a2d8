...
(function () {
  ...
  function focusables() { return panel ? Array.prototype.filter.call(panel.querySelectorAll(FOCUSABLE), function (node) { return node.getClientRects().length && !node.closest('[hidden],.is-collapsed'); }) : []; }
  function setInert(on) {
    if (on) {
      ...
      inertNodes.forEach(function (saved) {
  ...
  function expanded(value) {
    doc.querySelectorAll('[data-dsb8-menu-trigger],[data-dlx-open],.dsb-menu-toggle').forEach(function (node) {
      node.setAttribute('aria-expanded', value ? 'true' : 'false');
      node.setAttribute('aria-controls', drawer.id || 'delicat-drawer');
    });
  }
  ...
  function hide() {
    if (!drawer || !open) return;
    open = false; expanded(false); drag = null;
    entered();
  ...
  function bind() {
    ...
    html.classList.add('dlx-ready'); expanded(false);
    ...
    doc.addEventListener('pointerdown', function (e) {
      ...
      if (!trigger || open || e.isPrimary === false || (typeof e.button === 'number' && e.button !== 0)) return;
      /*
    ...
    doc.addEventListener('click', function (e) {
      ...
      if (!open) return;
      if (drawer.contains(t) && closest(t, '[data-dip-auth-open],[data-dl-open],a[href="#delicat-login"]')) { hide(); return; }
