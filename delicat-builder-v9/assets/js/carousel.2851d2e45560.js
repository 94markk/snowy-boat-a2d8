...
const renderBadge = (parent, badge) => {
	...
	/* The favourite mark, built the way the server builds it: the same shape
	   twice, an outline under a fill that springs out when it is tapped. The
	   geometry comes from the shared badge table, so a card materialised here
	   and a card rendered in PHP cannot drift apart. */
	const appendLikeMark = (parent, choice) => {
		const ns = 'http://www.w3.org/2000/svg';
		const name = heartIcon(choice) === 'heart-outline' ? 'heart' : heartIcon(choice);
		const markup = BADGE_ICONS[name] || BADGE_ICONS.heart || '';
		const d = /d="([^"]+)"/.exec(markup);
		if (!d) return;

		const wrap = document.createElement('span');
		wrap.className = 'delicat-like';
		wrap.setAttribute('aria-hidden', 'true');

		const svg = document.createElementNS(ns, 'svg');
		svg.setAttribute('class', 'delicat-like__mark');
		svg.setAttribute('viewBox', '0 0 24 24');
		svg.setAttribute('aria-hidden', 'true');
		svg.setAttribute('focusable', 'false');

		['delicat-like__outline', 'delicat-like__fill'].forEach((cls) => {
			const path = document.createElementNS(ns, 'path');
			path.setAttribute('class', cls);
			path.setAttribute('d', d[1]);
	...
	if (data.st?.t) {
		...
		if (Number(data.he) === 1) {
			...
			heart.type = 'button';
			heart.className = `delicat-product-card__bubble delicat-product-card__like${Number(data.hf) === 1 ? ' is-liked' : ''}`;
