...
(function () {
  ...
  function apply() {
    ...
    if (d.loggedIn) {
      if (d.wallet && d.wallet.text) { text('[data-dlx-wallet-balance]', d.wallet.text); text('[data-delicat-wallet-balance]', d.wallet.text); }
      text('[data-dlx-name]', d.name); text('[data-dlx-email]', d.email); text('[data-dlx-initial]', d.initial);
      var code = doc.querySelectorAll('[data-dlx-code]');
      for (var c = 0; c < code.length; c++) {
        if (d.code) { code[c].textContent = d.code; var btn = code[c].closest ? code[c].closest('[data-dlx-copy]') : null; if (btn) btn.setAttribute('data-dlx-copy', d.code); }
      }
    }
  ...
  function refresh(reason, force) {
    ...
    store.pending = fetch(cfg.url, { credentials: 'same-origin', headers: { 'Accept': 'application/json' }, cache: 'no-store' })
      ...
      .catch(function () { store.pending = null; return store.data; });
  ...
  var events = ['delicat:cart-changed', 'delicat:wallet-changed', 'delicat:express-added'];
