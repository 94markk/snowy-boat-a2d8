...
| --- | --- |
| Hide the supplier from the website | Storefront CSS/JS is inlined (the plugin path no longer appears in page source), imported titles/descriptions are scrubbed of provider names and URLs, catalog images are copied into your media library instead of hot-linked, fulfillment metadata is stripped from customer order views and emails, the callback route is removed from the public REST index, and imported SKUs use a neutral prefix |
...
| All features | Big-category catalog browsing, scheduled catalog/price/availability sync, an Orders screen with per-row re-sync, a Tools screen with manual jobs and environment diagnostics, a private rotating callback URL, broader webhook event handling, and five WP-CLI commands |
...
- **Page source.** Storefront styles and scripts are inlined, so `/wp-content/plugins/<folder>/` never appears
  in a product page. Without this, the plugin folder name is readable by anyone who views source.
...
- **Customer order views and emails.** Internal `_dst2t_*` metadata and any row that names the provider is
  removed from the customer-facing item meta. The customer still sees their own Player ID, server, and region.
- **REST.** The callback namespace is dropped from `/wp-json/` and a direct namespace index request 404s. The
  route itself keeps working.
...
immediately. The original `delicat-shop2topup/v1/webhook` route stays registered so upgrading never drops a
delivery — register the new URL upstream, then confirm deliveries before you consider the old one retired.

### What white-labelling does *not* do

Be realistic about the limits:

- **The plugin folder and its `readme.txt` still name the supplier on disk.** Anyone who guesses
  `/wp-content/plugins/delicat-shop2topup/readme.txt` can read it. Nothing links to it any more, but if you want
  this closed too, rename the plugin folder — the code uses no hard-coded folder name, so renaming works. You
  then update it by uploading a new ZIP under that same folder name.
...
- **70 isolated behavioral tests pass**: `php tests/regression.php` from this directory. They mock WordPress,
  WooCommerce, the database, and HTTP. The 25 tests from 1.1.0 are unchanged and still pass; 45 are new and
  cover white-labelling, dashboard masking, customer-facing metadata filtering, callback signature and event
  tolerance, the private callback token, wallet-balance caching and thresholds, settings round-tripping, stock
