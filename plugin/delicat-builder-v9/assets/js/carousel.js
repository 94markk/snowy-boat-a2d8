...
const renderBadge = (parent, badge) => {
	...
	/* pro.33: the like button, built to the same tree as
	   Delicat_Builder_V9_Heart_Engine::render_button(). The path comes from the
	   engine's config, so a card built here and a card built on the server draw
	   the same heart; the literal only covers a page whose config is missing.
	   heart.js owns every movement. */
	...
	const buildLikeButton = (productId, liked) => {
		const cfg = window.DelicaBuilderV9?.heart || {};
		...
		button.setAttribute('aria-label', liked
			? String(cfg.removeLabel || 'Retirer des favoris')
			: String(cfg.addLabel || 'Ajouter aux favoris'));
	...
	if (data.st?.t) {
		...
		if (Number(data.he) === 1) {
			mediaWrap.appendChild(buildLikeButton(productId, Number(data.hf) === 1));
		}
