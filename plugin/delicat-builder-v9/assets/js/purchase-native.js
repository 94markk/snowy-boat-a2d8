  ...
  function bindWalletCleanup() {
	...
	function bindCheckoutPresentation() {
		...
		function syncCheckoutPresentation() {
			...
			if (couponButton) {
				if (couponButton.disabled !== !coupon) couponButton.disabled = !coupon;
				if (coupon) addClass(body, 'dpn-coupon-proxy-ready');
				else removeClass(body, 'dpn-coupon-proxy-ready');
			}

			if (dockTotal && total && dockTotal.innerHTML !== total.innerHTML) dockTotal.innerHTML = total.innerHTML;
			if (!dock || !dockButton || !actual) {
				if (dockButton && !dockButton.disabled) dockButton.disabled = true;
				return;
			}
			...
			var disabled = !!actual.disabled || actual.getAttribute('aria-disabled') === 'true';
