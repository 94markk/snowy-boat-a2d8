...
(function () {
  ...
  function onDown(e) {
    if (!open || e.pointerType === 'mouse' || e.button || e.isPrimary === false) return;
    /*
  ...
  function onMove(e) {
    ...
    if (!drag.frame) {
      ...
      drag.frame = true;
      raf(function () {
        activeDrag.frame = false;
        if (drag === activeDrag && open) panel.style.transform = 'translateX(' + activeDrag.dx + 'px)';
      });
    }
  ...
  function bind() {
    ...
    doc.addEventListener('pointerdown', function (e) {
      ...
      if (!trigger || open || e.button || e.isPrimary === false) return;
      /*
