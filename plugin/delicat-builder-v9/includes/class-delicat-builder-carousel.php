...
final class Delicat_Builder_V9_Carousel {
	...
	public static function shortcode( array $atts = array() ): string {
		...
		/* pro.33: the like button's resting outline, chosen for contrast against
		 * the chip background on the server (no color-mix() on the storefront). */
		$carousel_vars[] = '--dbv9-like-ink:' . ( class_exists( 'Delicat_Builder_V9_Heart_Engine' ) ? Delicat_Builder_V9_Heart_Engine::ink_for( $heart_bg ) : '#1f2340' );
				...
				<div class="delicat-carousel__track" data-delicat-track tabindex="0" role="list">
					...
					if ( $managed_homepage && ! is_user_logged_in() ) {
						$live_cards_cache_key = self::deferred_payload_cache_key(
							$live_ids,
							array(
								'kind'               => 'live-card-html-v3', /* pro.34: cards cached without the like button */
				...
				<?php if ( $managed_homepage ) : ?>
					...
					$payload_cache_key = self::deferred_payload_cache_key(
						...
						array(
							...
							'heart_icon'         => $heart_icon,
							'show_cta'           => $show_cta ? 1 : 0,
							'like'               => 3, /* pro.34: payloads cached without the like button */
						)
					);
	...
	private static function deferred_card_payload(
		...
		/* pro.34: 'auto' and 'on' both mean "show the like button while the engine
		 * is on". 'auto' also required Design Studio to be active on the page,
		 * which it never is on the homepage, so no button was ever rendered there. */
	...
	private static function product_card( int $product_id, int $index, bool $show_price, bool $show_stock, string $tag_text = '', string $badge_mode = 'auto', string $status_badge_mode = 'auto', int $status_new_days = 30, int $status_sales_min = 10, string $style = '', string $heart_mode = 'auto', string $heart_icon = 'heart', bool $show_cta = true, bool $priority_image = false ): string {
		...
		>
			<div class="delicat-product-card__media-wrap">
				...
				<?php
				/* pro.33: the like button. One component, rendered by the engine that
				 * owns its markup; nothing else on the card draws a heart. */
				if ( $heart_enabled ) {
					echo Delicat_Builder_V9_Heart_Engine::render_button( $product_id, $is_favorite ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped inside render_button().
				}
				?>
			</div>
