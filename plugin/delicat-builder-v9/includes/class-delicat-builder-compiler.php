...
final class Delicat_Builder_V9_Compiler {
	...
	public static function maybe_recompile_for_version(): void {
		...
		try {
			...
			update_option( 'delicat_builder_v9_last_recompile', array(
				'at' => time(), 'version' => DELICAT_BUILDER_V9_VERSION,
				'recompiled' => $changed, 'failed' => array(),
				'error' => $failed > 0 ? 'Compilation incomplète; nouvelle tentative après le délai de reprise.' : '',
			), false );
			if ( $failed > 0 ) {
