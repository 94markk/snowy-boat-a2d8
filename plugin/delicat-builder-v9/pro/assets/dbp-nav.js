...
(function () {
  ...
  function documentPayload(text, response) {
    ...
    for (var j = 0; j < scripts.length; j++) {
      ...
      if (src) {
        ...
        var existing = script.id ? doc.getElementById(script.id) : null;
        if (existing && existing.src && normalise(existing.src) !== normalise(absolute)) throw new Error('script-version-changed');
  ...
  function loadScripts(list, inline, sequence, token) {
    indexExistingAssets();
  ...
  function scrollToHash(url) {
    ...
    var name;
    try { name = decodeURIComponent(url.hash.slice(1)); } catch (e) { return; }
    var target = doc.getElementById(name);
    if (!target && doc.getElementsByName) target = doc.getElementsByName(name)[0];
  ...
  function commit(payload, url, restoreTo, token, push) {
    ...
    return loadStyles(payload.styles || []).then(function () {
      ...
      if (token !== navToken) return;
      return loadScripts(payload.scripts || [], payload.inline || [], payload.sequence, token);
    }).then(function () {
      if (token !== navToken) return;
      ...
      if (typeof restoreTo !== 'number') scrollToHash(url);
    });
  }
  ...
  function navigate(url, options) {
    options = options || {};
    // Once script initialization starts it cannot be safely cancelled. A second
    // destination gets a fresh document instead of mixing two module lifecycles.
    if (commitInProgress) {
      navToken++;
