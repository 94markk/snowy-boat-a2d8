...
(function () {
  ...
  function fetchState(reason) {
    if (pending) {
      if (reason !== 'nav' && reason !== 'load' && !queuedRefresh) {
        queuedRefresh = pending.then(function () {
          queuedRefresh = null;
          return fetchState(reason);
        });
      }
      return queuedRefresh || pending;
    }
    ...
    pending = Promise.race([request, deadline]).then(function (next) {
  ...
  function paintWallet(next) {
    var balance = next.wallet && next.wallet.enabled ? String(next.wallet.balance || '') : '';
    var nodes = doc.querySelectorAll('[data-dbp-wallet]');
    for (var i = 0; i < nodes.length; i++) {
      text(nodes[i], balance);
    }
  }
  ...
  ['delicat:cart-changed', 'delicat:wallet-changed', 'delicat:auth-changed', 'delicat:currency-changed', 'delicat:express-added'].forEach(function (name) {
    doc.addEventListener(name, function () { fetchState('mutation'); });
  });
  ...
  doc.addEventListener('visibilitychange', function () {
    ...
    if (!state || Date.now() - lastFetch < 30000) return;
    fetchState('focus');
  });
