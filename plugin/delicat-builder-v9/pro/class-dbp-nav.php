 ...
 *
 * Pro.31 fetches ordinary cached HTML for eligible page swaps. Product,
 * account, wallet and checkout routes keep native WooCommerce navigation.
...
final class DBP_Nav {
	...
	public static function boot() {
		...
		add_action( 'wp_head', array( __CLASS__, 'document_generation' ), 1 );
		add_filter( 'body_class', array( __CLASS__, 'body_class' ) );
	}

	/** A cached document from another build must reload the complete shell. */
	public static function document_generation() {
		echo '<meta name="dbp-generation" content="' . esc_attr( DBP_Kernel::generation() ) . '">';
	}
	...
	public static function transform( $html ) {
		...
		if ( '' === $main ) {
			...
			return wp_json_encode( array( 'full' => true, 'reason' => 'no_main' ) ) ?: '{"full":true,"reason":"no_main"}';
		}
	...
	public static function enqueue() {
		if ( DBP_Kernel::is_fragment_request() || in_array( DBP_Kernel::route(), DBP_Kernel::locked_routes(), true ) ) {
			return;
		}
		...
		$config = array(
			...
			'home'        => home_url( '/' ),
			'locked'      => $locked,
			'blockParams' => array( 'add-to-cart', 'remove_item', 'undo_item', 'wc-ajax', 'download_file', 'logout', 'customer-logout', '_wpnonce', 'nonce', 'action', 'dip_action', 'key', 'token', 'code', 'payment_method', 'preview', 'delicat_builder_preview', 'dbp_nav', 'dbp_state', 'delicat_auth_reset', 'delicat_auth_recover', 'dip_auth_sync', 'session', 'sso', 'redirect_to' ),
