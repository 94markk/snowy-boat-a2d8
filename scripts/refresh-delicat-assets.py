...
chrome_path = os.path.join(ROOT, CHROME_BUNDLE)
missing_src = [c for c in CHROME_SOURCES if not os.path.exists(os.path.join(ROOT, c))]
if missing_src:
    raise SystemExit("chrome bundle sources missing: " + ", ".join(missing_src))

parts = [
    "/* Generated by scripts/refresh-delicat-assets.py. Do not edit by hand.",
    " * Concatenation, in enqueue order, of:",
]
for chrome_rel in CHROME_SOURCES:
    parts.append(" *   " + chrome_rel)
parts.append(" */")
for chrome_rel in CHROME_SOURCES:
    with open(os.path.join(ROOT, chrome_rel), encoding="utf-8") as fh:
        parts.append("\n/* --- " + chrome_rel + " --- */\n" + fh.read().strip())
chrome = "\n".join(parts) + "\n"
...
if os.path.exists(chrome_path):
    with open(chrome_path, encoding="utf-8") as fh:
        chrome_changed = fh.read() != chrome
if chrome_changed:
    with open(chrome_path, "w", encoding="utf-8") as fh:
        fh.write(chrome)
print("%s %s (%d bytes from %d sources)" % (
    "built   " if chrome_changed else "verified", CHROME_BUNDLE, len(chrome.encode("utf-8")), len(CHROME_SOURCES)))
